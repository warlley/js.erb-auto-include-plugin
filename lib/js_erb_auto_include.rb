  ...
  module Helpers
    module AssetTagHelper      
      ...
      def js_erb_auto_include_tag(namespace = "")
        file = File.join(RAILS_ROOT, "app", "views", namespace, controller.controller_name, "#{JsErbAutoInclude.update_action_name(action_name)}.js.erb")
        if File.exist?(file)
...
module JsErbAutoInclude
  ...
  def self.update_action_name(action_name)
  ...
  module InstanceMethods      
    ...
    def js_erb_auto_include
      @js_erb_auto_include_url = url_for(request.path_parameters.update({ :format => 'js', :action => JsErbAutoInclude.update_action_name(action_name) }))
    end
